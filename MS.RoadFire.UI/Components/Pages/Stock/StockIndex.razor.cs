...
namespace MS.RoadFire.UI.Components.Pages.Stock
{
    public partial class StockIndex
    {
        private List<StockDto>? Stocks { get; set; }
        private MudTable<StockDto> table = new();
        ...
        private async Task<TableData<StockDto>> LoadListAsync(TableState state, CancellationToken cancellationToken)
        {
            ...
            var responseHttp = await Repository.GetAsync<ResponseDto<List<StockDto>>>(url);
            if (responseHttp.Error)
            {
                var message = await responseHttp.GetErrorMessageAsync();
                Snackbar.Add(message!, Severity.Error);
                return new TableData<StockDto> { Items = [], TotalItems = 0 };
            }
