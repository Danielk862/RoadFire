...
namespace MS.RoadFire.UI.Components.Pages.Customer
{
    public partial class CustomerIndex
    {
        ...
        private MudTable<CustomerDto> table = new();
        ...
        private async Task<TableData<CustomerDto>> LoadListAsync(TableState state, CancellationToken cancellationToken)
        {
            ...
            if (responseHttp.Error)
            {
                var message = await responseHttp.GetErrorMessageAsync();
                Snackbar.Add(message!, Severity.Error);
                return new TableData<CustomerDto> { Items = [], TotalItems = 0 };
            }
        ...
        private async Task ShowModalAsync(int id = 0, bool isEdit = false)
        {
            ...
            if (isEdit)
            {
                ...
                dialog = await DialogService.ShowAsync<CustomerEdit>("Editar", parameters, options);
            }
            else
            {
                dialog = await DialogService.ShowAsync<CustomerCreate>("Nuevo", options);
            }
        ...
        private async Task DeleteAsync(CustomerDto customer)
        {
            var parameters = new DialogParameters
                {
                    { "Message", $"Estas seguro de borrar el cliente: {customer.FirstName} {customer.SecondName} {customer.Surname} {customer.SecondSurname}"}
                };
            ...
            var responseHttp = await Repository.DeleteAsync<ResponseDto<bool>>($"{baseUrl}/Delete/{customer.Id}");
