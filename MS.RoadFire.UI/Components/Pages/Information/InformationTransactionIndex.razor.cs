...
{
    public partial class InformationTransactionIndex
    {
        private List<TransactionDto>? Transaction { get; set; }
        private MudTable<TransactionDto> table = new();
        ...
        private async Task<TableData<TransactionDto>> LoadListAsync(TableState state, CancellationToken cancellationToken)
        {
            ...
            var responseHttp = await Repository.GetAsync<ResponseDto<List<TransactionDto>>>(url);
            if (responseHttp.Error)
            {
                var message = await responseHttp.GetErrorMessageAsync();
                Snackbar.Add(message!, Severity.Error);
                return new TableData<TransactionDto> { Items = [], TotalItems = 0 };
            }
        ...
        private async Task ShowDetailsPopup(TransactionDto transaction)
        {
            IDialogReference dialogRef = null!;

            var parameters = new DialogParameters
            {
                ["Transaction"] = transaction.TransactionDetailDtos,
            ...
            dialogRef = await DialogService.ShowAsync<InformationTransactionDetails>(transaction.Description, parameters, options);
        }
