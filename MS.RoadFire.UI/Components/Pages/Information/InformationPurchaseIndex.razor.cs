...
{
    public partial class InformationPurchaseIndex
    {
        private List<PurchaseDto>? Purchases { get; set; }
        private MudTable<PurchaseDto> table = new();
        ...
        private async Task<TableData<PurchaseDto>> LoadListAsync(TableState state, CancellationToken cancellationToken)
        {
            ...
            var responseHttp = await Repository.GetAsync<ResponseDto<List<PurchaseDto>>>(url);
            if (responseHttp.Error)
            {
                var message = await responseHttp.GetErrorMessageAsync();
                Snackbar.Add(message!, Severity.Error);
        ...
        private async Task ShowDetailsPopup(PurchaseDto purchase)
        {
            IDialogReference dialogRef = null!;

            var parameters = new DialogParameters
            {
                ["Purchase"] = purchase.PurchaseDetailsDtos,
            ...
            dialogRef  = await DialogService.ShowAsync<InformationPurchaseDetails>(purchase.Description, parameters, options);
        }
